...
import { GraduationCap, Briefcase, Github, Upload, ArrowRight, CheckCircle2 } from 'lucide-react'
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (mockData.success) {
          // Set up workspace with enhanced files
...
This project was automatically generated by GhostStack.

### API Endpoints

${data.data.endpoints.map((ep: any) => `- ${ep.method} ${ep.path}`).join('\n')}

### Getting Started

1. Click "Run" in the IDE to start the mock server
2. Test endpoints using the preview panel
3. Customize responses in \`ghost-server/index.js\`
...
`)
          ...
          toast({
            title: 'Success!',
            description: `Generated ${data.data.endpoints.length} API endpoint(s) from your repository`,
          })
          ...
          toast({
            title: 'Error',
            description: mockData.error || 'Failed to generate mock server. Please try again.',
          })
        }
      } else {
        ...
        toast({
          title: 'Error',
          description: data.error || 'Failed to scan repository. Please check the URL and try again.',
        })
      }
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: 'An error occurred while scanning the repository. Please try again.',
      })
    }
  }
